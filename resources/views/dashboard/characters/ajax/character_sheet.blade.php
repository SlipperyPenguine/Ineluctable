    ...
    <div class="box box-solid box-primary">
      ...
      <div class="box-body">
        <dl class="dl-horizontal">
          ...
          <dt>Skillpoints</dt>
          <dd>{{ number_format($skillpoints, 0,ineluctable\Services\Settings\SettingHelper::getSetting('decimal_seperator'),ineluctable\Services\Settings\SettingHelper::getSetting('thousand_seperator')) }}</dd>

          <dt>Wallet Balance</dt>
          <dd>{{ number_format($character->balance, 2, ineluctable\Services\Settings\SettingHelper::getSetting('decimal_seperator'),ineluctable\Services\Settings\SettingHelper::getSetting('thousand_seperator')) }} ISK</dd>
          ...
          @if (!empty($character_info->lastKnownLocation))
            <dt>Last Location:</dt>
            <dd>{{ $character_info->lastKnownLocation }}</dd>
          @endif
          ...
          @if (!empty($character_info->securityStatus))
            ...
            <dd>
              @if ($character_info->securityStatus < -5)
                <span class="text-red">{{ number_format($character_info->securityStatus, 2)  }}</span>
              @elseif ($character_info->securityStatus < -2)
                <span class="text-yellow">{{ number_format($character_info->securityStatus, 2) }}</span>
              @else
                <span class="text-green">{{ number_format($character_info->securityStatus, 2) }}</span>
              @endif
            </dd>
          @endif
        </dl>
      ...
      <div class="box-body">
        <dl class="dl-horizontal">
          ...
          @if ( strlen($character->typeName) > 0)
            ...
            <dd>
              @if ($skill_queue->last()->endTime)
                {{ $skill_queue->last()->endTime }} ({{ $skill_queue->last()->endTime->diffForHumans() }})
                @if (Carbon\Carbon::parse($skill_queue->last()->endTime)->lte(Carbon\Carbon::now()->addDay()))
          ...
          <dd>
            <ol>
              @foreach($skill_queue as $skill)
                <li>{{ $skill->skilltype->typeName }} {{ $skill->level }}</li>
              @endforeach
            </ol>
          </dd>
        </dl>
              ...
              <li>
                <i>
                  @if(strlen($jump_clone->cloneName))
                    ({{ $jump_clone->cloneName }})
                  @else
                    unnamed
                </i>
                @endif
                @ @if($jump_clone->location !=''){{ $jump_clone->location }} @else Clone Vat Bay @endif
              </li>
        ...
        <div class="box-body">
          <ul class="list-unstyled">
            ...
            @foreach($employment_history as $employment)
              ...
              <li>
                <img alt="image" class="img-circle" src="https://imageserver.eveonline.com/Corporation/{{$employment->corporationID}}_32.png" />
