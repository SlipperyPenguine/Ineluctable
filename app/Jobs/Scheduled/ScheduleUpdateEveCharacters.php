...
class ScheduleUpdateEveCharacters extends Job implements SelfHandling, ShouldQueue
{
    ...
    public function handle()
    {
        ...
        \Log::info('Started job UpdateEveCharacters', array('src' => __CLASS__));
        ...
        foreach (SeatKey::where('isOk', '=', 1)->get() as $key) {
            ...
            if ($access['type'] == 'Character') {
               ...
               $job = new ScheduleUpdateAccountCharacters($key->keyID, $key->vCode);
                $this->dispatch($job);
            }
        }
